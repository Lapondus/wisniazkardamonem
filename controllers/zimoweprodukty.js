...
module.exports.index = async (req, res) => {                               //Wszystkie produkty --- załadowuje stronę z tablicą (?) produktów (każdy z nich jest obiektem)
    const produkty = await ZimowyProdukt.find({});
    res.render('kolekcja_zimowa/index', { produkty });
}
module.exports.renderNewForm = (req, res) => {                                 //Z wszystkich produktów --> załadowuje formularz nowego produktu, który odsyła potem do strony nowego produktów
    res.render('kolekcja_zimowa/nowy');
}
module.exports.createProdukt = async (req, res) => {                              //Z formularza nowego produktu --> do strony nowego produktu (pokaz)  + try catch + middleware - walidator dla formularzy
    const produkt = new ZimowyProdukt(req.body.produkt);                                    // pobiera obiekt z req i zapisuje go jako nowy produkt
    ...
    res.redirect(`/kolekcja_zimowa/${produkt._id}`);                           //REDIRECT do pokaz (bo przy tworzeniu nie można się powołać na nieistniejące id (w przeciwieństwie do edytuj)
}
module.exports.showProdukt = async (req, res) => {                            //Z wszystkich produktów --> załadowuje stronę wybranego (poprzez link na nazwie)
    const produkt = await ZimowyProdukt.findById(req.params.id).populate({
        path: 'recenzje',
        populate: {
            path: 'author',
    ...
    res.render('kolekcja_zimowa/pokaz', { produkt });
}
module.exports.renderEditForm = async (req, res) => {                     //Ze strony danego produktu (pokaz)--> do formularza z edycją owego produktu
    const produkt = await ZimowyProdukt.findById(req.params.id)               // przy pomocy id elementu załadowanego w pokaz
    if (!produkt) {
        req.flash('error', 'Szukany produkt nie istnieje!');
        return res.redirect('/kolekcja_letnia');
    }
    res.render('kolekcja_zimowa/edytuj', { produkt });                           // uniwersalny szablon dla wszystkich edycji produktów
}
module.exports.updateProdukt = async (req, res) => {                           //Z formularza edycji produktu --> do strony uaktualnionego produktu (pokaz) + try catch + middleware - walidator dla formularzy
    const { id } = req.params;
    const produkt = await ZimowyProdukt.findByIdAndUpdate(id, { ...req.body.produkt });                 //Uaktualnienie produktu w bazie danych 
    ...
    res.redirect(`/kolekcja_zimowa/${id}`);                                    //REDIRECT do pokaz
}
module.exports.destroyProdukt = async (req, res) => {                        //Z strony pokazowej produktu do wszystkich produktów
    await ZimowyProdukt.findByIdAndDelete(req.params.id);                     //usunięcie produktu z bazy danych, pamiętać że mongoose middleware powiązane jest z specyficznym operatorem find... tutaj i zmiana jego wymaga zmiany middleware
    req.flash('success', 'Pomyślnie usunięto produkt!');
    res.redirect('/kolekcja_zimowa');                                          //REDIRECT do wszystkich produktów
}